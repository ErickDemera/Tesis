...
class Login extends DBConnection {
	...
	function login_agent(){
		...
		if($result->num_rows > 0){
			...
			if($res['status'] == 1){
				...
				$resp['status'] = 'failed';
				$resp['msg'] = 'Tu cuenta ha sido bloqueada.';
			}
		}else{
		$resp['status'] = 'failed';
		$resp['msg'] = 'Correo o contraseña incorrectos';
		}
		if($this->conn->error){
			$resp['status'] = 'failed';
	...
	function login_cliente(){
		extract($_POST);
		$stmt = $this->conn->prepare("SELECT * from clientes where email = ? and `password` = ? and delete_flag = 0 ");
		$password = md5($password);
		$stmt->bind_param('ss',$email,$password);
		$stmt->execute();
		$result = $stmt->get_result();
...
switch ($action) {
	...
	case 'logout_agent':
		...
		break;
	case 'login_cliente':
		echo $auth->login_cliente();
		break;
	case 'logout_cliente':
		echo $auth->logout_cliente();
		break;
	default:
		echo $auth->index();
		break;
